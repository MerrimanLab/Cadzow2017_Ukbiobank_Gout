...
# script for creating MALE ONLY normal gout GWAS
...
for (i in 1:1){
  ...
  control_ids[control_ids$ids %in% genotyped[genotyped$f.31.0.0 == 0, "f.eid"], 'avail'] <- FALSE
  ...
  for (case in genotyped[which(genotyped$goutaff == 1 & !is.na(genotyped$goutaff & genotyped$f.31.0.0 == 1)),"f.eid"]){
  ...
  write.table(new_fam_file[,c("FID","IID","PID","MID","SEX","AFF")], file = paste0("/run/user/1000/gvfs/smb-share:server=biocldap,share=scratch/merrimanlab/murray/working_dir/UkBio/controls_d/Male_only/chrallimpv1.fam_goutall_male",i), col.names=FALSE, row.names=FALSE, quote=FALSE, sep = ' ')
  ...
  # blank out non-white ethnicities
  new_fam_file[!(!is.na(new_fam_file$f.21000.0.0) & (new_fam_file$f.21000.0.0 == 1001 | new_fam_file$f.21000.0.0 == 1002 | new_fam_file$f.21000.0.0 == 1003)) , "AFF"] <- -9
  write.table(new_fam_file[,c("FID","IID","PID","MID","SEX","AFF")], file = paste0("/run/user/1000/gvfs/smb-share:server=biocldap,share=scratch/merrimanlab/murray/working_dir/UkBio/controls_d/Male_only/chrallimpv1.fam_gouthosp_male",i), col.names=FALSE, row.names=FALSE, quote=FALSE, sep = ' ')
  rm(c_id)
  ...
  # blank out non-white ethnicities
  new_fam_file[!(!is.na(new_fam_file$f.21000.0.0) & (new_fam_file$f.21000.0.0 == 1001 | new_fam_file$f.21000.0.0 == 1002 | new_fam_file$f.21000.0.0 == 1003)) , "AFF"] <- -9
  write.table(new_fam_file[,c("FID","IID","PID","MID","SEX","AFF")], file = paste0("/run/user/1000/gvfs/smb-share:server=biocldap,share=scratch/merrimanlab/murray/working_dir/UkBio/controls_d/Male_only/chrallimpv1.fam_goutwinnard_male",i), col.names=FALSE, row.names=FALSE, quote=FALSE, sep = ' ')
  rm(c_id)
  ...
  new_fam_file[!(!is.na(new_fam_file$f.21000.0.0) & (new_fam_file$f.21000.0.0 == 1001 | new_fam_file$f.21000.0.0 == 1002 | new_fam_file$f.21000.0.0 == 1003)) , "AFF"] <- -9
  table(new_fam_file$AFF, new_fam_file$f.21000.0.0, exclude=NULL)
  write.table(new_fam_file[,c("FID","IID","PID","MID","SEX","AFF")], file = paste0("/run/user/1000/gvfs/smb-share:server=biocldap,share=scratch/merrimanlab/murray/working_dir/UkBio/controls_d/Male_only/chrallimpv1.fam_goutself_male",i), col.names=FALSE, row.names=FALSE, quote=FALSE, sep = ' ')
  ...
  new_fam_file[!(!is.na(new_fam_file$f.21000.0.0) & (new_fam_file$f.21000.0.0 == 1001 | new_fam_file$f.21000.0.0 == 1002 | new_fam_file$f.21000.0.0 == 1003)) , "AFF"] <- -9
  table(new_fam_file$AFF, new_fam_file$f.21000.0.0, exclude=NULL)
  write.table(new_fam_file[,c("FID","IID","PID","MID","SEX","AFF")], file = paste0("/run/user/1000/gvfs/smb-share:server=biocldap,share=scratch/merrimanlab/murray/working_dir/UkBio/controls_d/Male_only/chrallimpv1.fam_goutself_ult_male",i), col.names=FALSE, row.names=FALSE, quote=FALSE, sep = ' ')
...
write.table(co_var_file, file = "/run/user/1000/gvfs/smb-share:server=biocldap,share=scratch/merrimanlab/murray/working_dir/UkBio/controls_d/Male_only/chrallimpv1.covar_male", col.names=TRUE, row.names=FALSE, quote=FALSE, sep = ' ')
