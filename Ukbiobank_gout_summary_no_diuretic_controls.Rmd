---
title: "UkBioBank Gout Summary Report - No Diuretic Controls"
...
ggplot(data = kt1, aes(x = gout, y = OR, colour = gout)) + geom_point() + facet_wrap(~SNP, scales = 'free_y') + geom_linerange(aes(x = gout, ymin = lower95, ymax = upper95)) +geom_hline(yintercept = 1) +theme_bw() + theme(axis.text.x = element_blank()) 

```


```{r, echo = FALSE, fig.align='centre', fig.width= 8, fig.height=8}
kt1_sig <- kt1[kt1$SNP %in% kt1[kt1$P < 1e-8, 'SNP'] , ]
kt1_sig$label <- paste("Chr",kt1_sig$CHR,'-', kt1_sig$grail_gene)
ggplot(data = kt1_sig, aes(x = gout, y = OR, colour = gout)) + geom_point() + facet_wrap(~label, scales = 'free_y') + geom_linerange(aes(x = gout, ymin = lower95, ymax = upper95)) +geom_hline(yintercept = 1) +theme_bw() + theme(axis.text.x = element_blank() + axis.ticks.x = element_blank()) 

```

```{r}
phipps <- data.frame(SNP =c('rs12498742','rs1260326', 'rs2231142', 'rs478607'), CHR =c(4,2,4, 11), BP =c( 9944052,27730940,89052323, 64478063), grail_gene = c('SLC2A9 ','GCKR ', 'ABCG2 ', 'SLC22A12 '), OR = c( 1/1.959, 1.314, 2.008, 1.239), lower95 = c( 1/1.615, 1.144, 1.681, 1.064), upper95 =c( 1/2.375, 1.509, 2.400, 1.444), P =c( 7.9e-12, 1.1e-4, 1.7e-14, 0.006))
phipps$SE <- NA
phipps$gout <- 'Phipps-Green et al 2016'
kt1_sig <- kt1[kt1$SNP %in% kt1[kt1$P < 1e-8, 'SNP'] , ]
kt1_sig <- rbind(kt1_sig, phipps)
kt1_sig$label <- paste("Chr",kt1_sig$CHR,'-', kt1_sig$grail_gene)

ggplot(data = kt1_sig, aes(x = gout, y = OR, colour = gout)) + geom_point() + facet_wrap(~label, scales = 'free_y') + geom_linerange(aes(x = gout, ymin = lower95, ymax = upper95)) +geom_hline(yintercept = 1) +theme_bw() + theme(axis.text.x = element_blank()) 
```
