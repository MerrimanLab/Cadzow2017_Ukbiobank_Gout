...
genotyped  %>% 
  ...
  mutate(sex = ifelse(f.31.0.0 == 0, "Female", ifelse(f.31.0.0 == 1, "Male",NA)), cc = ifelse(goutaff == 1, "Case", "Control")) %>% 
  mutate(mean(f.21001.0.0), sd(f.21001.0.0))  %>% as.data.frame
...
genotyped  %>% 
  ...
  mutate(sex = ifelse(f.31.0.0 == 0, "Female", ifelse(f.31.0.0 == 1, "Male",NA)), cc = ifelse(goutaff == 1, "Case", "Control")) %>% 
  mutate(mean(waist_height_ratio), sd(waist_height_ratio)) %>% as.data.frame
...
ukvariant <- read.table(paste0(scratch_dir,"/GWAS_all_controls/ukbio_kottgen_alleles.txt"), header=TRUE, stringsAsFactors = FALSE)
names(ukvariant)<- c('CHR','BP','SNP','A1','A2','TEST')
```



```{r, echo=FALSE, results = 'asis', eval = FALSE}
kottgen_table1 <- list(all =merge(all_kot[['sum_table']], ukvariant[,c("SNP","A1","A2")],by = "SNP", all.x = TRUE), winnard = merge(winnard_kot[['sum_table']], ukvariant[,c("SNP","A1","A2")],by = "SNP", all.x = TRUE), hosp = merge(hosp_kot[['sum_table']], ukvariant[,c("SNP","A1","A2")],by= "SNP", all.x = TRUE), self = merge(self_kot[['sum_table']], ukvariant[,c("SNP","A1","A2")],by = "SNP", all.x = TRUE), ult = merge(ult_kot[['sum_table']], ukvariant[,c("SNP","A1","A2")],by = "SNP", all.x = TRUE), self_ult = merge(self_ult_kot[['sum_table']], ukvariant[,c("SNP","A1","A2")],by = "SNP",all.x = TRUE), kottgen_gout = kot_t1_gout) 
...
```{r, echo = FALSE, fig.align='centre', fig.width= 8, fig.height=8, eval = FALSE}
